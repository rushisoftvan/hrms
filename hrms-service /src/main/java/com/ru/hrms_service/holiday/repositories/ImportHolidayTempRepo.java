...
//    void compareAndInsertHolidays(Long batchId);
    ...
    @Modifying
    @Transactional
    @Query(value = "CALL CompareAndInsertHolidays(:batchId)", nativeQuery = true)
    void compareAndInsertHolidays(@Param("batchId") Long batchId);

    @Query(value = """
        SELECT iht.remarks FROM import_holiday_temp iht 
    ...
    @Modifying
    @Transactional
    @Query(value = "CALL UpdateBatchRemarkInError(:batchId)", nativeQuery = true)
    void updateErrorMsgInBatchStatusRemarks();
